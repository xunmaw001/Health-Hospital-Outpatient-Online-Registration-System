...
public class YishengChatController {
    ...
    @Autowired
    private YishengLiuyanService yishengLiuyanService;//医生留言
    @Autowired
...
, "yishengId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            ...
            if(yisheng != null){
            BeanUtils.copyProperties( yisheng , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "yonghuId"
, "yishengId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
...
//        else if("医生".equals(role))
//            yishengChat.setYishengId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        if("".equals(yishengChat.getYishengChatIssueText()) || "null".equals(yishengChat.getYishengChatIssueText())){
                yishengChat.setYishengChatIssueText(null);
        }
        if("".equals(yishengChat.getYishengChatReplyText()) || "null".equals(yishengChat.getYishengChatReplyText())){
                yishengChat.setYishengChatReplyText(null);
        }
    ...
    public R detail(@PathVariable("id") Integer id, HttpServletRequest request){
        ...
        YishengChatEntity yishengChat = yishengChatService.selectById(id);
            if(yishengChat !=null){
                ...
                if(yonghu != null){
                    BeanUtils.copyProperties( yonghu , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "yonghuId"
, "yishengId"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setYonghuId(yonghu.getId());
                }
                //级联表
                    YishengEntity yisheng = yishengService.selectById(yishengChat.getYishengId());
